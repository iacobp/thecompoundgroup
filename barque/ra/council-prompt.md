...
3. A **Summary line** (1–2 sentences) for the §1 Summary lede section. Written as you would explain the forecast to a smart friend outside forecasting and pharma: no jargon, no calibration math, no initialisms the reader has to look up. Just *what Barque noticed today and why it might matter*.
...
**Section 1 — Summary.** *(public)* Jargon-free lede. One short paragraph per material forecast in the language of a smart non-specialist. No "PCAC / Brier / Cat 1 / posterior." The public log's entry point and the operator's 30-second skim. **Use H2 header `## Summary`** (renamed from "In Plain English" on 2026-05-09 for friendlier email reading).

**Section 2 — Signal detail.** *(public)* The rigorous layer: direct updates to pending forecasts with council reasoning, cited sources, and deltas. Augur picks. New forecast candidates awaiting operator approval. Three buckets — see the Signal Detail Section further below. **Email-friendliness:** within each forecast update, lead with a bold scannable line — e.g., **`forecast-id: holds 0.85`** or **`forecast-id: 0.71 → 0.74`** — so a phone reader can lock onto the moves without parsing every sentence. Bold key numbers and named entities within paragraphs.
...
**Section 5 — Work orders.** *(operational — committed to git, filtered out of `/barque/log` and out of the daily Resend email)* Tagged actions for sibling Compound products derived from today's signals. One block per product (GLP-1 Picks, GLP-1 Tracker, and siblings as they onboard) with structured prose lines the product's CCR routine can parse: "UPDATE page X because signal Y," "ADD disclosure to Z," "CONSIDER article on topic W." Machine-parseable but still readable. Never rendered on the public log; product routines query the committed brief file directly.

**CRITICAL — prose-and-TSV sync.** Every new action verb (UPDATE, ADD, CONSIDER, RESEARCH) you write in this section that is NOT already an open row in `work-orders.tsv` MUST be appended as a new row to `work-orders.tsv` in the same commit as the brief. The schema is: `date<TAB>id<TAB>source<TAB>source_id<TAB>product<TAB>action<TAB>target<TAB>reason<TAB>priority`. Use `ra-update` for source, today's date for source_id, and an id of the form `<product-slug>-<verb>-<short-descriptor>` (e.g. `glp1picks-update-foundayo-week6-rx`). Re-stating an existing pending item ("Prior pending: <id>") does NOT require a new row — that's a status callout, not a new order. Failing this sync means the work order is invisible to `dispatch-work-orders.yml` → no GitHub issue gets opened in the product repo → the order is silently dropped on the floor. If you cannot determine the right schema fields for a verb you want to write, prefer to NOT write that verb in Section 5 today and surface it instead as a Strategic Memo (§6) candidate for operator follow-up. The TSV is the contract; the prose is the explanation.

**Section 6 — Strategic memo.** *(private — operator-only, filtered out of public renders and out of the daily Resend email)* Portfolio-level candidates surfaced by today's evidence: new verticals to consider, products to kill or scale back, budget or editorial shifts. This section is committed to the brief file alongside the others, but the website parser explicitly filters it (and §5 Work Orders) from the public `/barque/log` render, AND the email-brief.yml workflow strips both §5 and §6 from the body sent via Resend. Because the barque repo is private, §6 content does not reach public surfaces. If this repo is ever made public, strip §6 from git history with `git filter-repo` before the visibility change. If nothing strategic surfaced, write "no strategic moves today" and move on — do not pad.
