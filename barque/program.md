...
- 2026-08-02 — **A stated falsifier is not a monitor (proposed, awaiting operator ratification).** For six consecutive runs Ra recorded `peptide-affiliate-share`'s falsifier as "a 503A peptide telehealth platform reaching affiliate-network standards before Dec 31" and never ran a query for one. The Precision Peptide Company had held LegitScript Healthcare Merchant Certification — the standard affiliate networks and ad platforms underwrite against — since 2026-06-23, and went nationwide DTC on 2026-07-07, sixteen days before the PCAC vote. The counter_narrative field was doing the work of a watchlist without being one. Proposed addition to the Ra run loop: where a forecast's falsifier names a **discrete, searchable event** (a launch, a filing, a certification, a partnership), that event gets an explicit query every run and the result is logged even when negative, rather than being restated as prose. Falsifiers that are diffuse ("sentiment shifts") are exempt; the rule applies only where a specific occurrence could be looked up and was not. This is an operator decision; the entry records the finding, not a protocol change.
