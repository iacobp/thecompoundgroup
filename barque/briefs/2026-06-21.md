...
- Prior pending: `hrtpicks-update-estrogen-shortage-worsening` (medium, filed 2026-06-19). Midi Health survey confirms ~50% fill difficulty across 8,000 women, 49 states. All five US manufacturers at full capacity. Projected 3-year shortage duration. Estrogen patch prescriptions up 184% since 2018, accelerating 20% in the last 8 months alone following the FDA black-box warning removal. Awaiting dispatch.
...
The Chainalysis fentanyl-to-peptide supply chain data is significant but does not create a buildable product opportunity — it is a journalism angle, not a consumer-health product gap. The estrogen patch shortage continues to worsen and is being addressed via HRT Picks. Pet GLP-1 readouts (MEOW-1, Akston AKS-562c) are approaching but no data has surfaced to trigger content-urgency work orders.

**Human peptide safety tracker** remains at **watch** — still fails threshold 2 (unverified search demand; Ahrefs plan-gated). No change from prior assessment.

Existing opportunities stable: `pet-peptide-safety-atlas` (proposed), `dog-longevity-tracker` (proposed), `hrt-telehealth-comparison` (built/synced as HRT Picks). No kill or scale-back signals. Quiet week strategically.
