...
- **UPDATE** `/estrogen-patch-shortage` page with worsening severity data: all 5 US manufacturers (Amneal, Noven, Sandoz, Mylan, Zydus) at full capacity; Midi Health survey of nearly 8,000 women across 49 states showing ~50% difficulty filling prescriptions; projected 3-year shortage duration per manufacturer warnings; worldwide scope per CBC; Dotti, Lyllana, and Vivelle-Dot backordered through late 2026. Sources: NBC News June 2026, Midi Health, SynergenX, CBC, Healthline. Prior completed: `hrtpicks-update-estrogen-patch-fda-denial` (2026-06-16).

## Strategic memo

**Mandatory opportunity pass:** no new revenue surface today.

The ECRI/ISMP white paper reinforces the **human peptide safety tracker** concept flagged yesterday as a watch item. Assessment unchanged: passes 4 of 5 New Vertical thresholds but fails threshold 2 (unverified search demand). The white paper does not change the demand verification status — it strengthens the supply-side thesis (institutional infrastructure for safety content exists) but consumer search intent for a safety-tracking tool remains unquantified. Stays at **watch**. Revisit post-PCAC July 23 when regulatory clarity may sharpen search intent.

Existing opportunities gated on stated falsifiers. No kill or scale-back signals. `pet-peptide-safety-atlas` (proposed), `dog-longevity-tracker` (proposed), `hrt-telehealth-comparison` (built/synced) — all stable.
